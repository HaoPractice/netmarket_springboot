...
public class HomePageController extends AbstractSecurityController {
  ...
  public ModelAndView getUserInfoByName() {
    ...
    if (isLoggedIn) {
      Customer customer = getUserInfoByUserId(authedUserId);
      modelAndView.addObject("user",
          new UserInfo(customer.getUsername(), customer.getUserId().toString()));
    }
    return modelAndView;
  }
  ...
  private Customer getUserInfoByUserId(Long authedUserId) {
    Customer customer = customerMapper.selectByPrimaryKey(authedUserId);
    return customer;
  }
